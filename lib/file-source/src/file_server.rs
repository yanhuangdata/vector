    ...
    E: FileSourceInternalEvents,
{
...

    {
        ...
        let mut existing_files = Vec::new();
        let mut files_not_completed: HashMap<FileFingerprint, u64> = HashMap::new();
        ...
        for path in self.paths_provider.paths().into_iter() {
            if let Some(file_id) = self.fingerprinter.get_fingerprint_or_log_error(
                ...
                files_not_completed.insert(file_id, file_size);
                existing_files.push((path, file_id));
            }
        }
        ...
        loop {
            ...
            let files_not_completed2 = files_not_completed.clone();
            for (file_id, file_size) in files_not_completed2 {
                let file_pos = checkpoints_view.get(file_id);
                match file_pos {
                    Some(pos) => {
                        if pos == file_size {
                            files_not_completed.remove(&file_id);
                        }
                    },
                    None => {}
                }
            }
            if files_not_completed.is_empty() {
                checkpoint_tx.send(true).unwrap();
                // drop(shutdown);
                break Ok(Shutdown);
            }

            stats.record("sleeping", start.elapsed());
            // println!("AAAAAbout to Exit, sleeping {}", start.elapsed().as_secs_f32());
            // break Ok(Shutdown);
            // break;
        }
    }
