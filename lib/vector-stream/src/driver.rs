    ...
    Svc::Response: DriverResponse,
{
                ...
                maybe_ready = poll_fn(|cx| service.poll_ready(cx)), if next_batch.is_some() => {
                    ...
                    while !batch.is_empty() {
                        ...
                        let events_byte_size =
                            i32::try_from(req.get_metadata().events_byte_size()).unwrap_or(i32::MAX);
                        let max_in_flight_batch_size_clone = max_in_flight_batch_size.clone();
                        max_in_flight_batch_size_clone.fetch_sub(events_byte_size, Ordering::SeqCst);
                        let fut = svc.call(req)
                            ...
                            .map(move |result| Self::handle_response(
                                ...
                                bytes_sent.as_ref(),
                                &events_sent,
                                &max_in_flight_batch_size_clone,
                            ))
    ...
    fn handle_response(
        ...
        max_in_flight_batch_size: &AtomicI32,
    ) {
        match result {
            Err(error) => {
        ...
        // restore the in-flight batch buffer size now the request has completed
        max_in_flight_batch_size.fetch_add(events_byte_size, Ordering::SeqCst);
    }
